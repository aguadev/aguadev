...
sub reports {
    ...
    $self->logDebug("$query");
    my $user_reports = $self->db()->queryhasharray($query);    
    $self->logDebug("user_reports", $user_reports);
    ...
    foreach my $hash( @$ownergroups )
    {
        ...
        $self->logDebug("$query");
        my $queryarray = $self->db()->queryarray($query);
    ...
    $self->logDebug("owner_reports", $owner_reports);
    
    
    $self->logDebug("used_owner_reports", $used_owner_reports);
    ...
    my $world_sources = $self->db()->queryhasharray($query);
    for my $world_source ( @$world_sources )
    {
        $self->logDebug("world_source", $world_source);
    ...
    my $json = $jsonObject->pretty->indent->encode($reports);
