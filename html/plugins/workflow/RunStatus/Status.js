...
// RUN WORKFLOW, QUIT IF ERROR, PROMPT FOR stopRun IF ALREADY RUNNING
	...
	var deferred = dojo.xhrPut({
		...
		handle: function (response) {
            thisObject.handleRunWorkflow(response, runner);
        }
	});
...
handleRunWorkflow : function (response, runner) {
			...
			if (response == null ) {
				...
				this.setNodeClass(runner.childNodes[start - 1], 'running');
			}
				...
				if ( response.error == "running" ) {
			        console.log("Status.runWorkflow    Doing this.confirmStopWorkflow()");
					this.displayWorkflowStatus("running");
					this.confirmStopWorkflow(project, workflow);
				}
				else {
				    Agua.toastError(response.error);
				}
				
				console.groupEnd("runStatus-" + this.id + "    runWorkflow");
				return;
			}
