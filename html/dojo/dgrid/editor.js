	...
	if((oldValue && oldValue.valueOf()) != (value && value.valueOf())){
		...
		if(column.field && row){
			...
			if(on.emit(cellElement, "dgrid-datachange", eventObject)){
				//console.log("dgrid.editor.setProperty    on.emit    eventObject:");
				//console.dir({eventObject:eventObject});
				//console.log("dgrid.editor.setProperty    on.emit    grid.updateDirty: " + grid.updateDirty);
				//console.log("dgrid.editor.setProperty    on.emit    column:");
