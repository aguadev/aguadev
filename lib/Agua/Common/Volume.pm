...
method loadSnapshot ($name, $id, $description) {
	$self->logDebug("IN PROGRESS");
...
=cut
	...
	my $ec2home		= 	$conf->getKey("aws", "EC2HOME");	
	my $javahome	= 	$conf->getKey("aws", "JAVAHOME");	
	$self->logError("ec2home not defined") and exit if not defined $ec2home;
	my $ec2 = "$ec2home/bin";
	$ENV{'EC2_HOME'} = $ec2home;
