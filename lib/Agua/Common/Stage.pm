...
WHERE username='$owner'\n};
	$query .= qq{AND project='$data->{project}'\n} if defined $data->{project};
...
sub getStagesByWorkflow {
	...
	$self->logNote("$$ workflowobject", $workflowobject);
...
sub updateStageSubmit {
	my $self		=	shift;
 	$self->logNote("$$ Common::updateStageSubmit()");
    my $json 			=	$self->json();
...
=cut
	...
	$self->logError("Could not insert stage $data->{name} into workflow $data->{workflow}") and exit if not $success;	
 	$self->logStatus("Inserted stage $data->{name} into workflow $data->{workflow}") if $success;	
}
...
$where};
	...
	my $success = $self->_addStage($data);
	return 0 if not defined $success or not $success;
...
sub addStageParametersForStage {
	my $self		=	shift;
	my $data		=	shift;
	$self->logNote("$$ data", $data);
...
AND appname='$appname'};
	...
	foreach my $parameter ( @$parameters )
	{
		...
		$parameter->{value} =~ s/%project%/$project/g if defined $project;
		$parameter->{value} =~ s/%workflow%/$workflow/g if defined $workflow;
...
=cut
	...
	my $self		=	shift;
 	$self->logNote("$$ Common::removeStage()");
