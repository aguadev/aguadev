...
#### CREATE SGEADMIN AND ADMIN USERS
method createUsers {
	#$self->sgeUser();
...
method sgeUser {
#### CREATE SGE USER LINUX ACCOUNT

print "\n\n\n";
print "****************************************************************\n";
print "********            CREATE SGE USER ACCOUNT            *********\n";
print "****************************************************************\n";
print "\n\n\n";

    #### SGE USERNAME
	my $username = $self->conf()->getKey("agua", "SGEUSER");
	$self->logDebug("username", $username);
	$username = $self->_inputValue("SGE username", $username);	
    $self->logError("username not defined") if not defined $username;
	$self->logDebug("FINAL username", $username);

	#### SGE USERID
	my $userid = $self->conf()->getKey("agua", "SGEUSERID");
	$self->logDebug("username", $username);
	$userid = $self->_inputValue("SGE userid", $userid);	
    $self->logError("userid not defined") if not defined $userid;
	$self->logDebug("FINAL userid ", $userid);
	...
	$self->_addLinuxDaemon($object);

print "\n\n\n";
print "****************************************************************\n";
print "**********    SGE USER ACCOUNT SUCCESSFULLY CREATED   **********\n";
print "****************************************************************\n";
print "\n\n\n";
...
\n};
	...
	#### CHECK FOR QUIT
    print "\nExiting\n\n" and exit if not $self->yes("Do you want to continue (Y/N)?");
}
...
method setMysqlRoot {
	...
	print "\n";
	my $resetroot = qq{Do you want to reset the MySQL root password (Y/N)?};
...
method apps {
    ...
    my $message = "You can set application paths or use the default paths. Do you want to use the default paths (Y/N)?";
	print "$message\n";
    return if yes($message);
