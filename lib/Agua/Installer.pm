...
#### THEN, INSTALL RABBIT.JS AND START server.js
	...
	$self->setConf() if not defined $self->get_conf();
	
	my $installdir 	= $self->get_installdir();
	my $install	=	$self->get_conf()->getKey("install", undef);
	print "Agua::Installer::installExchange    install: $install\n";		
	...
	print "Agua::Installer::installExchange    installdir: $installdir\n";		
	print "Agua::Installer::installExchange    nodeurl: $nodeurl\n";
	print "Agua::Installer::installExchange    amqpversion: $amqpversion\n";
	print "Agua::Installer::installExchange    rabbitversion: $rabbitversion\n";
	print "Agua::Installer::installExchange    socketversion: $socketversion\n";
...
##### SET RabbitMQ SIGNING KEY
	...
	my $tempdir = $self->get_tempdir() || "/tmp";
	$self->runCommands([
...
#### START THE RABBITMQ SERVER
	...
	print "Agua::Installer::startRabbitMq    command: $command\n";

	return `$command`;
}
sub rabbitMqIsRunning {
...
sub startExchange {
	...
	my $installdir = $self->get_installdir();
	$self->runCommands(["node /usr/local/lib/node_modules/rabbit.js/example/socketio/server.js"]);
	#system("`node $installdir/apps/node-amqp/node_modules/rabbit.js/example/socketio/server.js`");
}
#### CONFIG
sub copyConf {
...
sub isInstalled {
	...
	print "Agua::Installer::isInstalled    packagemanager: $packagemanager\n";
...
#### SET COMMANDS TO BE RUN AT STARTUP FROM STARTUP SCRIPT
	...
	print "Agua::Installer::setStartupScript    startupfile", $startupfile if defined $startupfile;
...
sub installBioDbSam {
	...
	print "Agua::Installer::installBioDbSam    command: $command\n";
...
#### REPLACE apache2.conf, sites-available/default AND envvars
	...
	my $urlprefix	=	$self->get_urlprefix();
	print "replaceApacheConfs   urlprefix: $urlprefix\n";
...
sub installPerlMods {
	...
	#### INSTALL libxml2-dev FOR Net::RabbitFoot DEPENDENCY XML::LibXML
...
sub linkDirectories {
    ...
    my $linksfile=	"$Bin/resources/agua/links.txt";
    print "Agua::Installer::linkDirectories    Agua::Installer::linkDirectories(linksfile)\n";
    print "Agua::Installer::linkDirectories    linksfile: $linksfile\n";
    my $contents = $self->fileContents($linksfile);
    print "Agua::Installer::linkDirectories    linksfile is empty: $linksfile\n" if not defined $contents or not $contents;
...
sub addLink {
    ...
    print "Agua::Installer::addLink    command: $command\n"; 
	`$command`;
